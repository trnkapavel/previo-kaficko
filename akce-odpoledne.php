<?php
// Samostatná stránka pro odpolední akci
$landing_slot = 'afternoon';
$landing_config = [
    'variant' => 'afternoon',
    'hero_cta_slot' => 'afternoon',
    'page_title_suffix' => ' | Odpolední MeetUp',
];

// předvyplnění typu účasti na registrační stránce
$prefill_slot = 'afternoon';

$debug = false;

// 1. Načtení dat
$json_data = file_get_contents('data-odpoledne.json');
$data = json_decode($json_data, true);

if (!$data) {
    $data = [
        'city' => 'Liberec', 'date' => '24. března 2026', 'time' => '13:00',
...
// 2. Kontext odpolední varianty
$landing_variant = 'afternoon';
$hero_cta_slot = $landing_config['hero_cta_slot'] ?? 'afternoon';
$page_title_suffix = $landing_config['page_title_suffix'] ?? ' | Odpolední MeetUp';

// Program: odpolední blok (PRO/LITE)
$program_single = $data['program'] ?? [];
$program_subtitle = 'Odpoledne: PRO/LITE (pro klienty)';
$fixed_type = 'prolite';

// 3. Hero texty – fallback na promo texty
$hero_title = !empty($data['hero_title']) ? $data['hero_title'] : (!empty($data['promo_title']) ? $data['promo_title'] : 'Odpolední blok: Previo v praxi');
$hero_text  = !empty($data['hero_text'])  ? $data['hero_text']  : (!empty($data['promo_text']) ? $data['promo_text'] : 'Pro stávající klienty Previa. Novinky v systému, e-Turista pro pokročilé a praktické ukázky automatizace.');
